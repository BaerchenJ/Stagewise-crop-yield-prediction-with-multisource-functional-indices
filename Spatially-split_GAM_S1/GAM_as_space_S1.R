...
y_train = range01(traindata$Ystar)
...
s1gam <- gam(y_train ~ s(CR, k = k, bs = "ps") +
                       s(GDD, k = k, bs = "ps") +
                       s(SR, k = k, bs = "ps") +
                       s(VP, k = k, bs = "ps") , data = trainwimean, method = "REML")
...
# RMSE test
rmse_test_ori <- sqrt(mean((ytest - testpreds)^2))
rmse_test <- sqrt(mean((testdata$Ystar - back(testpreds, testdata$Ystar))^2))

#res[[k/5]] = list(k = k, r2_valid)
#}

#write.xlsx(data.frame(do.call(rbind, res)), "TuneK\\GAM_as_space_S1.xlsx")
...
write.xlsx(data.frame(r2list, rmselist), "S1Res\\GAM_as_space_S1_result.xlsx")

# Fit

traindf <- data.frame(traindata[,1:5], Ynorm = y_train, Fit = s1gam$fitted.values, Resi = s1gam$residuals)

write.xlsx(traindf, "S1Res\\GAM_as_space_S1_train.xlsx")

validdf <- data.frame(validdata[,1:5], Ynorm = yvalid, Fit = validpreds, Resi = yvalid - validpreds)

write.xlsx(validdf, "S1Res\\GAM_as_space_S1_valid.xlsx")

testdf <- data.frame(testdata[,1:5], Ynorm = ytest, Fit = testpreds, Resi = ytest - testpreds)

write.xlsx(testdf, "S1Res\\GAM_as_space_S1_test.xlsx")
