...
fundat_wi <- import("wholesepanorm_spatial.xlsx")

# Spatially-split Train, Validation, Test sets were SEPARATELY scaled in advance, while combined in one data table.
...
wholedf <- cbind(fundat_wi[,1:5], Fit)

library(openxlsx)

write.xlsx(data.frame(wholedf), "BSVFMOOS_spatial.xlsx")
