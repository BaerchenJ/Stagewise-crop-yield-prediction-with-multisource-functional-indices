...
resi_train_43 <- import("S1Res\\GAM_as_space_S1_train.xlsx")
...
s2gam <- gam(y2_train ~ s(SM, k = k, bs = "ps") +
               
                        s(PR, k = k, bs = "ps"), data = trainspmean, method = "REML")
...
resi_test_43 <- import("S1Res\\GAM_as_space_S1_test.xlsx")
...
#write.xlsx(resdf, "TuneK\\GAM_as_space_S2.xlsx")
...
write.xlsx(data.frame(r2vec_s1, r2vec_s2, rmsevec_s1, rmsevec_s2), "S2Res\\GAM_as_space_S2_result.xlsx")
...
write.xlsx(traindf, "S2Res\\GAM_as_space_S2_train.xlsx")

validdf <- data.frame(resi_valid_8, 
                      Fit2 = validpreds, 
                      Resi2 = y2_valid - validpreds)

write.xlsx(validdf, "S2Res\\GAM_as_space_S2_valid.xlsx")

testdf <- data.frame(resi_test_8, 
                     Fit2 = testpreds, 
                     Resi2 = y2_test - testpreds)

write.xlsx(testdf, "S2Res\\GAM_as_space_S2_test.xlsx")
