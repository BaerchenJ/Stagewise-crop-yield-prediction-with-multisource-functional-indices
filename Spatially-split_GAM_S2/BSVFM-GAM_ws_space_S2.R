...
resi_train_43 <- import("S1Res\\BSVFM_oos_spatial_train.xlsx")
...
rmse_train_s1 <- sqrt(mean((resi_train_8$Yori - back(ytrain - s2y1, Yori_train))^2))

# s2

rmse_train_s2 <- sqrt(mean((resi_train_8$Yori - back(ytrain - gamw26$residuals, Yori_train))^2))
...
rmse_valid_s1 <- sqrt(mean((resi_valid_8$Yori - back(yvalid - y2_valid, Yori_valid))^2))

# s2

rmse_valid_s2 <- sqrt(mean((resi_valid_8$Yori - back(yvalid - y2_valid + validpreds, Yori_valid))^2))

# R2 test

resi_test_43 <- import("S1Res\\BSVFM_oos_spatial_test.xlsx")
...
rmse_test_s1 <- sqrt(mean((resi_test_8$Yori - back(ytest - y2_test, Yori_test))^2))

# s2

rmse_test_s2 <- sqrt(mean((resi_test_8$Yori - back(ytest - y2_test + testpreds, Yori_test))^2))
...
#write.xlsx(resdf, "TuneK\\BSVFM-GAM_ws_S2.xlsx")
...
write.xlsx(data.frame(r2vec_s1, r2vec_s2, rmsevec_s1, rmsevec_s2), "S2Res\\BSVFM-GAM_ws_S2_result.xlsx")
...
write.xlsx(traindf, "S2Res\\BSVFM-GAM_ws_S2_train.xlsx")

validdf <- data.frame(resi_valid_8, 
                      Fit2 = validpreds, 
                      Resi2 = y2_valid - validpreds)

write.xlsx(validdf, "S2Res\\BSVFM-GAM_ws_S2_valid.xlsx")

testdf <- data.frame(resi_test_8, 
                     Fit2 = testpreds, 
                     Resi2 = y2_test - testpreds)

write.xlsx(testdf, "S2Res\\BSVFM-GAM_ws_S2_test.xlsx")
