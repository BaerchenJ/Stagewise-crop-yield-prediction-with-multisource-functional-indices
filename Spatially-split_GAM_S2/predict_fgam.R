...
predfgam <- function (object, newdata, type = "response", se.fit = FALSE, 
        ...
        for (trm in trms) {
          ...
          if (is.af) {
            ...
            if (J != length(af$xind) & type != "lpmatrix") {
              stop(paste("Provided data for functional covariate", 
                         cov, "does not have same observation times as original data", 
                         sep = ""))
            }
            L <- matrix(af$L[1, ], nobs, J, byrow = T)
            tmat <- matrix(af$xind, nobs, J, byrow = TRUE)
            if (grepl(paste(cov, "\\.[ot]mat", 
                            sep = ""), deparse(af$call$x))) {
              if (length(attr(newdata, "L"))) {
                if (type != "lpmatrix") {
                  ...
                  if (sum(dim(as.matrix(attr(newdata, 
                                             "L"))) == dim(as.matrix(newdata[[cov]]))) != 
                      2) {
                    warning(paste("Supplied L matrix for", 
                                  cov, "is not the same dimension as the matrix of observations and will be ignored", 
                                  sep = ""))
                  }
                  else {
              ...
              if (length(attr(newdata, "tmat"))) {
                if (type != "lpmatrix") {
                  ...
                  if (sum(dim(as.matrix(attr(newdata, 
                                             "tmat"))) == dim(as.matrix(newdata[[cov]]))) != 
                      2) {
                    warning(paste("Supplied tmat matrix for", 
                                  cov, "is not the same dimension as the matrix of observations and will be ignored", 
                                  sep = ""))
                  }
                  else {
              ...
              #}
              if (type == "lpmatrix") 
                newdata[[cov]] <- as.vector(newdata[[cov]])
  ...
  if (type %in% c("terms", "iterms") & !all(terms %in% 
    ...
    if (terms %in% unlist(varmap)) {
      tnames <- c(names(object$smooth), attr(object$pterms, 
                                             "term.labels"))
      terms <- tnames[grep(paste(string, "[,\\.\\)]", 
                                 sep = ""), tnames)]
    }
    else {
